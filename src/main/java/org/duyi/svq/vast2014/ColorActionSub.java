...
/**
 * 这个color action 是为了与父亲的coloraction 保持颜色上的对应
 * @author	duyi
 * @date	2014年6月27日
 */
public class ColorActionSub extends ColorAction {
     ...
     */
    public ColorActionSub(String group, String dataField, 
    ...
    public ColorActionSub(String group, String string, int nominal,
			String strokecolor, Map map, ColorMap colorMap) {
		this(group, string, nominal, strokecolor);
		this.newOmap = map;
		this.newCmap = colorMap;
	}
...
//        return m_cmap.getColor(idx.doubleValue());
        Integer idx = (Integer)newOmap.get(item.get(m_dataField));
        return newCmap.getColor(idx.doubleValue());
////    	m_omap = 
